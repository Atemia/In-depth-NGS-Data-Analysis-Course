...
* Generate graphs comparing samples using `deepTools`
...
```
$ srun -n 4 --mem 2000 -p classroom --pty bash
...
```
$ multiBamSummary bins --ignoreDuplicates -p $SLURM_NTASKS \
...
#### 3. Sample fingerprint - `plotfingerprint` tool

The purpose of this plot is to determine whether we can easily see a difference
between the background signal from the input and the IP samples. This samples
the BAM files directly and plots a profile of cumulative read coverage for each
file: first a sampling a collection of bins is assessed for read coverage and
sorted, with the cumulative sum plotted.

![fingerprint](http://deeptools.readthedocs.io/en/latest/_images/QC_fingerprint.png)

We will pull data only from the region we assessed to cut down on processing
time; the recommendation is to limit this to a chromosome or a set # of bins

```
$ plotFingerprint --ignoreDuplicates -p $SLURM_NTASKS \
  -n 50000 --region chr12:1000000:33800000 \
  -b ../bowtie2/*Input*aln.bam ../bowtie2/*Nanog*aln.bam\
  --plotFile deepTools_fingerprint_Nanog.png \
  -T "Fingerprint of read counts - Nanog" \
  --outQualityMetrics deepTools_fingerprint_metrics.tab \
  --labels Input_Rep1 Input_Rep2 Nanog_Rep1 Nanog_Rep2
```
